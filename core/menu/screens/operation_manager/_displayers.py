...
def _display_capital_stats(summary: Dict[str, Any], operacion: Operacion, side: str, current_price: float):
    ...
    # 1. Calcular todas las métricas necesarias
    ...
    capital_operativo = operacion.balances.operational_margin
    capital_en_uso = operacion.balances.used_margin

    safe_division = utils.safe_division if utils else lambda n, d: n / d if d != 0 else 0.0
    roi_total = safe_division(equity_actual_vivo - capital_inicial, capital_inicial) * 100
    ...
    # 2. Ensamblar el diccionario de datos con la nueva estructura unificada
    data = {
        "--- CAPITAL ---": "---",
        "Capital Inicial (Base ROI)": f"${capital_inicial:.2f}",
        "Capital Operativo (Actual)": f"${capital_operativo:.2f}",
        "Capital en Uso": f"${capital_en_uso:.2f}",
        "--- RENDIMIENTO ---": "---",
        ...
        "ROI Total (Vivo)": f"{get_color(roi_total)}{roi_total:+.2f}%{reset}",
        "--- CONTADORES ---": "---",
    ...
    for key, value in data.items():
        if "---" in key:
            print(_create_box_line(f"\033[90m{key}\033[0m", box_width, 'center'))
        else:
