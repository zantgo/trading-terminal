# =============== INICIO ARCHIVO: core/strategy/pm_state.py (COMPLETO Y MODIFICADO) ===============
"""
...
def get_executor() -> Optional[Any]:
    return _executor

def get_leverage() -> float:
    return _leverage
    
def get_max_logical_positions() -> int:
    return _max_logical_positions

def get_initial_base_position_size() -> float:
    return _initial_base_position_size_usdt

def get_dynamic_base_size(side: str) -> float:
    return _current_dynamic_base_size_long if side == 'long' else _current_dynamic_base_size_short
...
def get_total_transferred_profit() -> float:
    return _total_transferred_profit
...
def set_initial_config(op_mode: str, live_mode: bool, exec_instance: Any, lev: float, max_pos: int, base_size: float, stop_event: Optional[threading.Event]):
    ...
    _operation_mode = op_mode
    _is_live_mode = live_mode
    _executor = exec_instance
    _leverage = lev
    _max_logical_positions = max_pos
    _initial_base_position_size_usdt = base_size
    _stop_loss_event = stop_event
...
def set_dynamic_base_size(long_size: float, short_size: float):
    ...
    _current_dynamic_base_size_long = long_size
    _current_dynamic_base_size_short = short_size
...
def add_realized_pnl(side: str, pnl: float):
    ...
    if side == 'long':
        _total_realized_pnl_long += pnl
    else:
        _total_realized_pnl_short += pnl
...
def get_global_sl_pct() -> Optional[float]:
    return _global_stop_loss_roi_pct

def get_global_tp_pct() -> Optional[float]:
    return _global_take_profit_roi_pct

def is_session_tp_hit() -> bool:
    return _session_tp_hit
...
def get_session_time_limit() -> Dict[str, Any]:
    return {"duration": _session_max_duration_minutes, "action": _session_time_limit_action}

def set_session_time_limit(duration: int, action: str):
    global _session_max_duration_minutes, _session_time_limit_action
    _session_max_duration_minutes = duration
    _session_time_limit_action = action
...
def start_new_trend(side: str):
    global _current_trend_side, _trades_in_current_trend, _initial_pnl_at_trend_start, _trend_take_profit_hit
    _current_trend_side = side
    _trades_in_current_trend = 0
    _initial_pnl_at_trend_start = _total_realized_pnl_long + _total_realized_pnl_short
    _trend_take_profit_hit = False
...
def set_manual_mode(mode: str, limit: Optional[int]):
    global _manual_mode, _manual_trade_limit, _manual_trades_executed
    _manual_mode = mode
    _manual_trade_limit = limit if limit and limit > 0 else None
    _manual_trades_executed = 0

def increment_manual_trades():
    global _manual_trades_executed
    _manual_trades_executed += 1
# =============== FIN ARCHIVO: core/strategy/pm_state.py (COMPLETO Y MODIFICADO) ===============