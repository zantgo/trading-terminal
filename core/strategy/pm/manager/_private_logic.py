...
class _PrivateLogic:
    ...
    def _start_operation(self, reason: str):
        ...
        if operacion and operacion.estado == 'EN_ESPERA':
            params_to_update = {
                'estado': 'ACTIVA',
                'tiempo_inicio_ejecucion': datetime.datetime.now(datetime.timezone.utc)
            }
            self._om_api.create_or_update_operation(params_to_update)
            self._memory_logger.log(f"OPERACIÓN INICIADA: Modo '{operacion.tendencia}' activado. Razón: {reason}", "INFO")

    def _end_operation(self, reason: str):
        """Finaliza la operación activa y la resetea a un estado NEUTRAL EN_ESPERA."""
        operacion = self._om_api.get_operation()
        if operacion and operacion.estado == 'ACTIVA':
            tendencia_anterior = operacion.tendencia
            self._memory_logger.log(f"OPERACIÓN FINALIZADA: Modo '{tendencia_anterior}' terminado. Razón: {reason}", "INFO")

            capital_actual = operacion.capital_inicial_usdt + operacion.pnl_realizado_usdt
            
            base_size = operacion.tamaño_posicion_base_usdt
            max_pos = operacion.max_posiciones_logicas
            ...
            op_neutral = Operacion(
                ...
                posiciones_activas=operacion.posiciones_activas
            )
            self._om_api.create_or_update_operation(asdict(op_neutral))
    ...
    def _can_open_new_position(self, side: str) -> bool:
        operacion = self._om_api.get_operation()
        if self._session_tp_hit or not operacion: return False
    ...
    def _open_logical_position(self, side: str, entry_price: float, timestamp: datetime.datetime):
        ...
        if result and result.get('success'):
            ...
            if new_pos_obj:
                current_op = self._om_api.get_operation()
                new_positions = current_op.posiciones_activas
                new_positions[side].append(new_pos_obj)
                self._om_api.create_or_update_operation({'posiciones_activas': new_positions})
    ...
    def _update_trailing_stop(self, side, position_obj: LogicalPosition, index: int, current_price: float):
        ...
        # Enviar el estado actualizado de todas las posiciones a la API
        self._om_api.create_or_update_operation({'posiciones_activas': operacion.posiciones_activas})

    def _close_logical_position(self, side: str, index: int, exit_price: float, timestamp: datetime.datetime, reason: str) -> bool:
        operacion = self._om_api.get_operation()
        if not self._executor or not operacion: return False
        ...
        if result and result.get('success', False):
            ...
            # Enviamos todos los cambios en una sola llamada a la API
            params_to_update = {
