...
def initialize(
    ...
    if not config or not _utils or not hasattr(_utils, 'safe_float_convert'):
    ...
    try:
        _default_base_position_size_usdt_config = max(0.0, _utils.safe_float_convert(getattr(config, 'POSITION_BASE_SIZE_USDT', 10.0)))
    ...
    if is_live:
        ...
        try:
            if profit_acc_name and profit_acc_name in real_balances_data:
                ...
                uta_profit_avail = _utils.safe_float_convert(unified_prof.get('totalAvailableBalance'), 0.0) if unified_prof else 0.0
                fund_profit_wallet = _utils.safe_float_convert(funding_prof.get('USDT', {}).get('walletBalance'), 0.0) if funding_prof else 0.0
        ...
        if target_long_acc_name in real_balances_data:
            ...
            if unified_long_data:
                ...
                if usdt_coin_data:
                    real_operational_long_margin_api = _utils.safe_float_convert(usdt_coin_data.get('walletBalance'), 0.0)
                else: 
                    real_operational_long_margin_api = _utils.safe_float_convert(unified_long_data.get('totalWalletBalance'), 0.0)
        ...
        if target_short_acc_name in real_balances_data:
            ...
            if unified_short_data:
                ...
                if usdt_coin_data_short:
                    real_operational_short_margin_api = _utils.safe_float_convert(usdt_coin_data_short.get('walletBalance'), 0.0)
                else:
                    real_operational_short_margin_api = _utils.safe_float_convert(unified_short_data.get('totalWalletBalance'), 0.0)
