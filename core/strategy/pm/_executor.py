...
try:
    from core.logging import _memory_logger
except ImportError:
    class MemoryLoggerFallback:
        def log(self, msg, level="INFO"): print(f"[{level}] {msg}")
    _memory_logger = MemoryLoggerFallback()
...
class PositionExecutor:
    ...
    def __init__(self,
        ...
        try:
            ...
            _memory_logger.log("ERROR CRITICO [PositionExecutor]: No se pudo importar pm_state.", level="ERROR")
        ...
        if self._is_live_mode and not self._live_manager: 
            _memory_logger.log("WARN [PositionExecutor Init]: Modo Live pero 'live_manager' no proporcionado. Transferencias API fallarán.", level="WARN")
        # --- CÓDIGO ANTERIOR (COMENTADO) ---
        ...
        try:
            ...
            # --- INICIO MODIFICACIÓN: Usar memory_logger en lugar de print ---
            _memory_logger.log(f"WARN [PositionExecutor Init]: Error cacheando config: {e}. Usando defaults.", level="WARN")
            # --- CÓDIGO ANTERIOR (COMENTADO) ---
            # print(f"WARN [PositionExecutor Init]: Error cacheando config: {e}. Usando defaults.")
            # --- FIN MODIFICACIÓN ---
        _memory_logger.log(f"[PositionExecutor] Inicializado. Modo Live: {self._is_live_mode}", level="INFO")
    ...
    def execute_open(self, side: str, entry_price: float, timestamp: datetime.datetime, margin_to_use: Optional[float] = None, size_contracts_str_api: Optional[str] = None ) -> Dict[str, Any]:
        ...
        _memory_logger.log(f"OPEN [{side.upper()}] -> Solicitud para abrir @ {entry_price:.{self._price_prec}f}", level="INFO")
        # --- CÓDIGO ANTERIOR (COMENTADO) ---
        # memory_logger.log(f"--- INICIO APERTURA [{side.upper()}] Px: {entry_price:.{self._price_prec}f} ---")
        # --- FIN MODIFICACIÓN ---
        
        if side not in ['long', 'short']: result['message'] = f"Lado inválido '{side}'."; _memory_logger.log(f"ERROR [Exec Open]: {result['message']}", level="ERROR"); return result
        if not isinstance(entry_price, (int, float)) or entry_price <= 0: result['message'] = f"Precio entrada inválido {entry_price}."; _memory_logger.log(f"ERROR [Exec Open]: {result['message']}", level="ERROR"); return result
        if margin_to_use is None and size_contracts_str_api is None: result['message'] = "Debe proveer 'margin_to_use' o 'size_contracts_str_api'."; _memory_logger.log(f"ERROR [Exec Open]: {result['message']}", level="ERROR"); return result
        if not all([self._position_state, self._balance_manager, self._position_calculations, self._position_helpers, self._utils, self._pm_state]): result['message'] = "Faltan dependencias internas."; _memory_logger.log(f"ERROR [Exec Open]: {result['message']}", level="ERROR"); return result

        size_contracts_final_float = 0.0; margin_used_final = 0.0; qty_precision_used = self._qty_prec
        try:
            if size_contracts_str_api is None:
                if not isinstance(margin_to_use, (int, float)) or margin_to_use <= 1e-6: result['message'] = f"Margen a usar inválido ({margin_to_use})."; _memory_logger.log(f"ERROR [Exec Open]: {result['message']}", level="ERROR"); return result
                margin_used_final = margin_to_use
                _memory_logger.log(f"  Calculando tamaño desde Margen: {margin_used_final:.4f} USDT", level="DEBUG")
                ...
                if not calc_qty_result['success']: result['message'] = calc_qty_result['error']; _memory_logger.log(f"ERROR [Exec Open]: {result['message']}", level="ERROR"); return result
                ...
                _memory_logger.log(f"  Usando tamaño pre-calculado: {size_contracts_str_api}", level="DEBUG")
                size_contracts_final_float = self._utils.safe_float_convert(size_contracts_str_api, 0.0)
                if size_contracts_final_float <= 1e-12: result['message'] = f"Tamaño provisto inválido ({size_contracts_str_api})."; _memory_logger.log(f"ERROR [Exec Open]: {result['message']}", level="ERROR"); return result
                margin_used_final = self._utils.safe_division(size_contracts_final_float * entry_price, self._leverage, default=0.0)
                if margin_used_final <= 0: _memory_logger.log(f"WARN [Exec Open]: Margen recalculado para tamaño provisto es {margin_used_final}.", level="WARN")
            _memory_logger.log(f"  Tamaño Final: {size_contracts_final_float:.{qty_precision_used}f} ({size_contracts_str_api} API), Margen: {margin_used_final:.4f} USDT", level="DEBUG")
        except Exception as e: result['message'] = f"Excepción calculando tamaño/margen: {e}"; _memory_logger.log(f"ERROR [Exec Open]: {result['message']}", level="ERROR"); traceback.print_exc(); return result
        ...
        try:
            if self._is_live_mode:
                _memory_logger.log(f"  Ejecutando Apertura LIVE API...", level="DEBUG")
                ...
                if api_response and api_response.get('retCode') == 0:
                    execution_success = True; api_order_id = api_response.get('result', {}).get('orderId', 'N/A'); _memory_logger.log(f"  -> ÉXITO API: Orden Market {order_side_api} aceptada. OrderID: {api_order_id}")
                else:
                    ret_code = api_response.get('retCode', -1) if api_response else -1; ret_msg = api_response.get('retMsg', 'N/A') if api_response else 'N/A'; result['message'] = f"Fallo API orden Market {order_side_api}. Code={ret_code}, Msg='{ret_msg}'"; _memory_logger.log(f"  -> ERROR API: {result['message']}", level="ERROR")
            else:
                _memory_logger.log(f"  Ejecutando Apertura BACKTEST (Simulada)...", level="DEBUG")
                execution_success = True; api_order_id = None
                _memory_logger.log(f"  -> ÉXITO Simulado.", level="DEBUG")
            if execution_success:
                self._balance_manager.decrease_operational_margin(side, margin_used_final)
        except Exception as exec_err: result['message'] = f"Excepción durante ejecución: {exec_err}"; _memory_logger.log(f"ERROR [Exec Open]: {result['message']}", level="ERROR"); traceback.print_exc(); execution_success = False
        ...
        if execution_success:
            try:
                ...
                if not add_ok:
                    ...
                    _memory_logger.log(f"ERROR SEVERE [Exec Open]: {result['message']}", level="ERROR")
                    result['success'] = False
                    return result

                _memory_logger.log(f"  Position State: Posición lógica ...{logical_position_id[-6:]} añadida (datos estimados).", level="DEBUG")
                
                _memory_logger.log(f"  Actualizando estado físico agregado...", level="DEBUG")
                ...
                _memory_logger.log(f"  -> Estado físico agregado {side.upper()} recalculado.", level="DEBUG")

            except Exception as state_err: result['message'] = f"Ejecución OK pero falló post-proceso: {state_err}"; _memory_logger.log(f"ERROR SEVERE [Exec Open]: {result['message']}", level="ERROR"); traceback.print_exc(); result['success'] = False; return result
        ...
        if result['success']:
            _memory_logger.log(f"OPEN [{side.upper()}] -> ÉXITO. Tamaño: {size_contracts_final_float:.{qty_precision_used}f}, Margen: {margin_used_final:.2f} USDT", level="INFO")
        else:
            _memory_logger.log(f"OPEN [{side.upper()}] -> FALLO. Razón: {result['message']}", level="ERROR")
        # --- CÓDIGO ANTERIOR (COMENTADO) ---
    ...
    def execute_close(self, side: str, position_index: int, exit_price: float,
        ...
        _memory_logger.log(f"CLOSE [{side.upper()} Idx:{position_index}] -> Solicitud para cerrar @ {exit_price:.{self._price_prec}f} (Razón: {exit_reason})", level="INFO")
        # --- CÓDIGO ANTERIOR (COMENTADO) ---
        # memory_logger.log(f"--- INICIO CIERRE [{side.upper()} Idx: {position_index}, Razón: {exit_reason}] ---")
        # --- FIN MODIFICACIÓN ---

        if side not in ['long', 'short']: result['message'] = "Lado inválido."; _memory_logger.log(f"ERROR [Exec Close]: {result['message']}", level="ERROR"); return result
        if not isinstance(exit_price, (int, float)) or exit_price <= 0: result['message'] = "Precio salida inválido."; _memory_logger.log(f"ERROR [Exec Close]: {result['message']}", level="ERROR"); return result
        if not all([self._position_state, self._balance_manager, self._position_calculations, self._position_helpers, self._utils]): result['message'] = "Faltan dependencias internas."; _memory_logger.log(f"ERROR [Exec Close]: {result['message']}", level="ERROR"); return result
        ...
        try:
            ...
            if not (0 <= position_index < len(open_positions)): result['message'] = f"Índice {position_index} fuera de rango."; _memory_logger.log(f"ERROR [Exec Close]: {result['message']}", level="ERROR"); return result
            ...
            if size_contracts_for_calc <= 1e-12: result['message'] = f"Tamaño lógico <= 0 para pos ID ...{pos_id_for_log[-6:]}. Considerado ya cerrado."; _memory_logger.log(f"WARN [Exec Close]: {result['message']}", level="WARN"); result['success'] = True; return result
            ...
            if not format_qty_result['success']: result['message'] = f"Error formateando Qty para API ({format_qty_result['error']}) Pos ID ...{pos_id_for_log[-6:]}."; _memory_logger.log(f"ERROR [Exec Close]: {result['message']}", level="ERROR"); return result
            size_contracts_str_api = format_qty_result['qty_str']
        except Exception as data_err: result['message'] = f"Excepción obteniendo datos/formateando: {data_err}"; _memory_logger.log(f"ERROR [Exec Close]: {result['message']}", level="ERROR"); traceback.print_exc(); return result

        execution_success = False; api_order_id_close = None; ret_code: Optional[int] = None; ret_msg: Optional[str] = None
        try:
            if self._is_live_mode:
                _memory_logger.log(f"  Ejecutando Cierre LIVE API (ReduceOnly)...", level="DEBUG")
                if not self._live_operations: raise RuntimeError("Live Operations no disponible")
                target_account = getattr(self._config, 'ACCOUNT_LONGS' if side == 'long' else 'ACCOUNT_SHORTS', None); main_account = getattr(self._config, 'ACCOUNT_MAIN', 'main'); account_to_use = target_account if target_account and target_account in self._live_manager.get_initialized_accounts() else main_account
                if account_to_use not in self._live_manager.get_initialized_accounts(): raise RuntimeError(f"Cuenta operativa '{account_to_use}' no inicializada.")
                ...
                if api_response and api_response.get('retCode') == 0:
                    execution_success = True; api_order_id_close = api_response.get('result', {}).get('orderId', 'N/A'); _memory_logger.log(f"  -> ÉXITO API: Orden Cierre Market {close_order_side_api} aceptada. OrderID: {api_order_id_close}")
                else:
                    ...
                    result['message'] = f"Fallo API orden Cierre Market {close_order_side_api}. Code={ret_code}, Msg='{ret_msg}'"; _memory_logger.log(f"  -> ERROR API: {result['message']}", level="ERROR")
                    if ret_code == 110001: execution_success = True; _memory_logger.log("  WARN [Exec Close]: Orden/Posición no encontrada (110001). Permitiendo limpieza lógica.", level="WARN")
            else:
                _memory_logger.log(f"  Ejecutando Cierre BACKTEST (Simulado)...", level="DEBUG")
                execution_success = True; api_order_id_close = None
                _memory_logger.log(f"  -> ÉXITO Simulado.", level="DEBUG")
        except Exception as exec_err: result['message'] = f"Excepción durante ejecución: {exec_err}"; _memory_logger.log(f"ERROR [Exec Close]: {result['message']}", level="ERROR"); traceback.print_exc(); execution_success = False
        ...
        if execution_success:
            try:
                ...
                if removed_pos_data:
                    ...
                    _memory_logger.log(f"  Position State: Posición lógica ...{pos_id_for_log[-6:]} removida.", level="DEBUG")
                elif ret_code == 110001:
                    remove_ok = True; _memory_logger.log(f"  INFO [Exec Close]: Pos lógica idx {position_index} (ID: ...{pos_id_for_log[-6:]}) no encontrada, consistente con API 110001.")
                else:
                    result['message'] = f"Ejecución OK pero falló remover pos lógica idx {position_index} (ID: ...{pos_id_for_log[-6:]})."; _memory_logger.log(f"ERROR SEVERE [Exec Close]: {result['message']}", level="ERROR"); result['success'] = False; return result
                ...
                _memory_logger.log(f"  Cálculos PNL: Neto={pnl_net_usdt:+.{self._pnl_prec}f}, Reinv={amount_reinvested_op_margin:.{self._pnl_prec}f}, Transf={amount_transferable_profit:.{self._pnl_prec}f}", level="DEBUG")
                ...
                if remove_ok and removed_pos_data:
                    ...
                    _memory_logger.log("  Balance Manager: No se modifica margen operativo (posición no encontrada).", level="DEBUG")

                _memory_logger.log(f"  Actualizando estado físico agregado post-remoción...", level="DEBUG")
                open_positions_now = self._position_state.get_open_logical_positions(side);
                if open_positions_now:
                    ...
                    _memory_logger.log(f"  -> Estado físico {side.upper()} recalculado (pos restantes: {len(open_positions_now)}).", level="DEBUG")
                else:
                    self._position_state.reset_physical_position_state(side)
                    _memory_logger.log(f"  -> Estado físico {side.upper()} reseteado (no quedan pos lógicas).", level="DEBUG")
                ...
                if self._closed_position_logger and hasattr(self._closed_position_logger, 'log_closed_position'):
                    try: self._closed_position_logger.log_closed_position(log_data_final)
                        ...
                        # --- INICIO MODIFICACIÓN: Usar memory_logger en lugar de print ---
                        _memory_logger.log(f"ERROR [Exec Close]: Fallo log pos cerrada ID {pos_id_for_log}: {log_e}", level="ERROR")
                        # --- CÓDIGO ANTERIOR (COMENTADO) ---
                        # print(f"ERROR [Exec Close]: Fallo log pos cerrada ID {pos_id_for_log}: {log_e}")
                        # --- FIN MODIFICACIÓN ---

            except Exception as state_err: result['message'] = f"Ejecución OK pero falló post-proceso: {state_err}"; _memory_logger.log(f"ERROR SEVERE [Exec Close]: {result['message']}", level="ERROR"); traceback.print_exc(); result['success'] = False; return result
        ...
        if result['success']:
            ...
            _memory_logger.log(f"CLOSE [{side.upper()} Idx:{position_index}] -> ÉXITO. PNL Neto: {pnl_net:+.{self._pnl_prec}f} USDT", level=log_level)
        else:
            _memory_logger.log(f"CLOSE [{side.upper()} Idx:{position_index}] -> FALLO. Razón: {result['message']}", level="ERROR")
        # --- CÓDIGO ANTERIOR (COMENTADO) ---
    ...
    def execute_transfer(self, amount: float, from_account_side: str) -> float:
        _memory_logger.log(f"Solicitando transferencia de {amount:.4f} desde {from_account_side.upper()}", level="DEBUG")
        transferred_amount_api_or_sim = 0.0
        if amount <= 1e-9:
            _memory_logger.log("  Monto <= 0, omitida.", level="DEBUG")
            return 0.0

        try:
            if self._is_live_mode:
                if not self._live_manager or not self._live_operations or not self._config:
                    _memory_logger.log("ERROR [Exec Transfer Live]: live_manager/live_ops/config no disponibles.", level="ERROR"); return 0.0
                ...
                if not from_acc_name or not to_acc_name: _memory_logger.log(f"ERROR [Exec Transfer Live]: Cuenta origen/destino no definida.", level="ERROR"); return 0.0
                loaded_uids = getattr(self._config, 'LOADED_UIDS', {}); from_uid = loaded_uids.get(from_acc_name); to_uid = loaded_uids.get(to_acc_name)
                if not from_uid or not to_uid: _memory_logger.log(f"ERROR [Exec Transfer Live]: UIDs no encontrados.", level="ERROR"); return 0.0
                try: from_uid_int = int(from_uid); to_uid_int = int(to_uid)
                except ValueError: _memory_logger.log(f"ERROR [Exec Transfer Live]: UIDs inválidos (no int).", level="ERROR"); return 0.0
                ...
                if not session_for_call: _memory_logger.log(f"ERROR [Exec Transfer Live]: Sesión API no disponible.", level="ERROR"); return 0.0

                for attempt in range(MAX_TRANSFER_RETRIES + 1):
                    _memory_logger.log(f"  API Call Attempt #{attempt + 1}/{MAX_TRANSFER_RETRIES + 1}: create_universal_transfer(...)", level="DEBUG")
                    resp = None
                    try:
                        resp = self._live_manager.create_universal_transfer(
                         ...
                         _memory_logger.log(f"    ERROR [Exec Transfer]: Excepción en llamada API (Intento {attempt + 1}): {api_call_err}", level="ERROR")
                         if attempt < MAX_TRANSFER_RETRIES: time.sleep(TRANSFER_RETRY_DELAY); continue
                         else: _memory_logger.log("    Fallo API después de máximos reintentos por excepción.", level="ERROR"); break
                    if resp and resp.get('retCode') == 0:
                        transfer_id = resp.get('result', {}).get('transferId', 'N/A')
                        _memory_logger.log(f"    -> ÉXITO API Transfer: ID={transfer_id}, Status={resp.get('result',{}).get('status','?')}")
                        transferred_amount_api_or_sim = amount
                        break
                    elif resp:
                        ret_code_t = resp.get('retCode', -1); ret_msg_t = resp.get('retMsg', 'N/A')
                        _memory_logger.log(f"    -> FALLO API Transfer (Intento {attempt + 1}): Code={ret_code_t}, Msg='{ret_msg_t}'", level="WARN")
                        non_retryable_codes = [131200, 131001, 131228, 10003, 10005, 10019, 131214, 131204, 131206, 131210]
                        if ret_code_t in non_retryable_codes: _memory_logger.log("      Error no recuperable.", level="WARN"); transferred_amount_api_or_sim = 0.0; break
                        if attempt < MAX_TRANSFER_RETRIES: time.sleep(TRANSFER_RETRY_DELAY)
                        else: _memory_logger.log("    Fallo API después de máximos reintentos.", level="ERROR"); transferred_amount_api_or_sim = 0.0
                    else:
                        _memory_logger.log(f"    -> FALLO API Transfer (Intento {attempt + 1}): No se recibió respuesta.", level="WARN")
                        if attempt < MAX_TRANSFER_RETRIES: time.sleep(TRANSFER_RETRY_DELAY)
                        else: _memory_logger.log("    Fallo API sin respuesta después de máximos reintentos.", level="ERROR"); transferred_amount_api_or_sim = 0.0
            else:
                _memory_logger.log("  Ejecutando Transferencia BACKTEST (Simulada)...", level="DEBUG")
                if not self._balance_manager: _memory_logger.log("ERROR [Exec Transfer BT]: balance_manager no disponible.", level="ERROR"); return 0.0
                sim_success = self._balance_manager.simulate_profit_transfer(from_account_side, amount)
                if sim_success:
                    _memory_logger.log(f"  -> ÉXITO Simulación Transfer: {amount:.4f} USDT reflejado en BalanceManager.", level="DEBUG")
                    transferred_amount_api_or_sim = amount
                else: _memory_logger.log(f"  -> FALLO Simulación Transfer.", level="ERROR"); transferred_amount_api_or_sim = 0.0
        except Exception as e:
            _memory_logger.log(f"ERROR [Exec Transfer]: Excepción general: {e}", level="ERROR"); traceback.print_exc(); transferred_amount_api_or_sim = 0.0

        _memory_logger.log(f"  Monto Efectivamente Transferido (API o Sim): {transferred_amount_api_or_sim:.4f} USDT", level="DEBUG")
    ...
    def sync_physical_state(self, side: str):
        ...
        if not self._is_live_mode: 
            _memory_logger.log("WARN [Sync State]: Solo aplicable en modo Live.", level="WARN")
            return
        if not all([self._position_state, self._live_operations, self._config, self._utils, self._position_helpers, self._live_manager]):
            _memory_logger.log("WARN [Sync State]: Faltan dependencias.", level="WARN")
            return
        if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": 
            _memory_logger.log(f"  ... Sincronizando estado físico {side.upper()} con API ...", level="DEBUG")
        # --- CÓDIGO ANTERIOR (COMENTADO) ---
        ...
        try:
            ...
            if account_to_use not in self._live_manager.get_initialized_accounts(): 
                _memory_logger.log(f"    ERROR SYNC: Cuenta '{account_to_use}' no inicializada.", level="ERROR")
                return
            ...
            if physical_pos_raw is None:
                _memory_logger.log(f"    WARN [Sync State]: No se recibieron datos de posición desde API para {side.upper()}. No se puede sincronizar.", level="WARN")
                return
            # --- CÓDIGO ANTERIOR (COMENTADO) ---
            ...
            if physical_state_data:
                ...
                if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": 
                    ...
                    _memory_logger.log(f"    SYNC OK: Estado físico {side.upper()} actualizado desde API.", level="DEBUG")
                    # --- CÓDIGO ANTERIOR (COMENTADO) ---
                ...
                if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": 
                    # --- INICIO MODIFICACIÓN: Usar memory_logger ---
                    _memory_logger.log(f"    SYNC OK: No hay posición física {side.upper()} en API. Estado reseteado.", level="DEBUG")
                    # --- CÓDIGO ANTERIOR (COMENTADO) ---
            ...
            if self._print_updates and getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG":
                # --- INICIO MODIFICACIÓN: Usar memory_logger ---
                _memory_logger.log(f"    --- Estado Físico {side.upper()} Actualizado (JSON): ---", level="DEBUG")
                try:
                    ...
                    _memory_logger.log(json.dumps(state_to_print, indent=2), level="DEBUG")
                except Exception as json_e:
                    _memory_logger.log(f"    ERROR [Sync State]: Format JSON: {json_e}\n    Estado crudo: {current_physical_state}", level="ERROR")
                _memory_logger.log(f"    -------------------------------------------------", level="DEBUG")
                # --- CÓDIGO ANTERIOR (COMENTADO) ---
            ...
            # --- INICIO MODIFICACIÓN: Usar memory_logger ---
            _memory_logger.log(f"    ERROR SYNC: Excepción durante sincronización {side.upper()}: {sync_err}", level="ERROR")
            # --- CÓDIGO ANTERIOR (COMENTADO) ---
