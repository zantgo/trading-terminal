# =============== INICIO ARCHIVO: core/strategy/position_executor.py (CORREGIDO FINAL V2) ===============
"""
...
# --- INICIO MODIFICACIÓN: Reducir delays y reintentos para operaciones no críticas ---
MAX_TRANSFER_RETRIES = 1 # 1 intento inicial + 1 reintento = 2 intentos en total
TRANSFER_RETRY_DELAY = 0.2 # Reducido a 200 milisegundos
# --- FIN MODIFICACIÓN ---
...
class PositionExecutor:
    ...
    def execute_open(self, side: str, entry_price: float, timestamp: datetime.datetime, margin_to_use: Optional[float] = None, size_contracts_str_api: Optional[str] = None ) -> Dict[str, Any]:
        ...
        try:
            if size_contracts_str_api is None:
                ...
                if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print(f"  Calculando tamaño desde Margen: {margin_used_final:.4f} USDT")
                ...
                if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print(f"  Usando tamaño pre-calculado: {size_contracts_str_api}")
            ...
            if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print(f"  Tamaño Final: {size_contracts_final_float:.{qty_precision_used}f} ({size_contracts_str_api} para API), Margen Usado Estimado: {margin_used_final:.4f} USDT")
        ...
        try:
            if self._is_live_mode:
                if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print(f"  Ejecutando Apertura LIVE API...")
                ...
                if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print(f"  Ejecutando Apertura BACKTEST (Simulada)...")
                execution_success = True; api_order_id = None
                if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print(f"  -> ÉXITO Simulado.")
        ...
        if execution_success:
            try:
                ...
                if not add_ok:
                    ...
                    print(f"ERROR SEVERE [Exec Open]: {result['message']}")
                ...
                if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG":
                    ...
                    if self._print_updates:
                        print("\n  --- ESTADO POST-APERTURA (Lógica Añadida) ---")
                        self._position_state.display_logical_table(side)
                        print("  " + "-"*60)
                
                if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print(f"  Actualizando estado físico agregado...")
                ...
                if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print(f"  -> Estado físico agregado {side.upper()} recalculado.")
    ...
    def execute_close(self, side: str, position_index: int, exit_price: float,
        ...
        try:
            if self._is_live_mode:
                if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print(f"  Ejecutando Cierre LIVE API (ReduceOnly)...")
                if not self._live_operations: raise RuntimeError("Live Operations no disponible")
                target_account = getattr(self._config, 'ACCOUNT_LONGS' if side == 'long' else 'ACCOUNT_SHORTS', None); main_account = getattr(self._config, 'ACCOUNT_MAIN', 'main'); account_to_use = target_account if target_account and target_account in self._live_manager.get_initialized_accounts() else main_account
                if account_to_use not in self._live_manager.get_initialized_accounts(): raise RuntimeError(f"Cuenta operativa '{account_to_use}' no inicializada.")
                ...
                if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print(f"  Ejecutando Cierre BACKTEST (Simulado)...")
        ...
        if execution_success:
            try:
                ...
                if removed_pos_data:
                    ...
                    if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print(f"  Position State: Posición lógica ...{pos_id_for_log[-6:]} removida.")
                ...
                if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print(f"  Cálculos PNL: Neto={pnl_net_usdt:+.{self._pnl_prec}f}, Reinv Op.Margin={amount_reinvested_op_margin:.{self._pnl_prec}f}, Transf. Profit={amount_transferable_profit:.{self._pnl_prec}f}")
                ...
                if remove_ok and removed_pos_data:
                    ...
                    if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print("  Balance Manager: No se modifica margen operativo (posición no encontrada).")

                if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print(f"  Actualizando estado físico agregado post-remoción...")
                open_positions_now = self._position_state.get_open_logical_positions(side);
                if open_positions_now:
                    ...
                    if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print(f"  -> Estado físico {side.upper()} recalculado (pos restantes: {len(open_positions_now)}).")
                else:
                    self._position_state.reset_physical_position_state(side)
                    if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print(f"  -> Estado físico {side.upper()} reseteado (no quedan pos lógicas).")
                ...
                if self._print_updates and getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG":
                    print("\n  --- ESTADO POST-CIERRE (Lógica Eliminada) ---")
                    self._position_state.display_logical_table(side)
                    print("  " + "-"*60)
    ...
    def execute_transfer(self, amount: float, from_account_side: str) -> float:
        ...
        if amount <= 1e-9:
            if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print("  INFO [Exec Transfer]: Monto <= 0, omitida.")
            return 0.0
        ...
        try:
            if self._is_live_mode:
                ...
                for attempt in range(MAX_TRANSFER_RETRIES + 1):
                    if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print(f"  API Call Attempt #{attempt + 1}/{MAX_TRANSFER_RETRIES + 1}: create_universal_transfer(...)")
                    resp = None
                    try:
                ...
                if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print("  Ejecutando Transferencia BACKTEST (Simulada)...")
                ...
                if sim_success:
                    if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print(f"  -> ÉXITO Simulación Transfer: {amount:.4f} USDT reflejado en BalanceManager.")
                    transferred_amount_api_or_sim = amount
        ...
        if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print(f"  Monto Efectivamente Transferido (API o Sim): {transferred_amount_api_or_sim:.4f} USDT")
    ...
    def sync_physical_state(self, side: str):
        ...
        try:
            ...
            if physical_state_data:
                ...
                if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print(f"    SYNC OK: Estado físico {side.upper()} actualizado desde API.")
            else:
                self._position_state.reset_physical_position_state(side)
                if getattr(self._config, 'LOG_LEVEL', 'INFO') == "DEBUG": print(f"    SYNC OK: No hay posición física {side.upper()} en API. Estado reseteado.")
...
# =============== FIN ARCHIVO: core/strategy/position_executor.py (CORREGIDO FINAL V2) ===============