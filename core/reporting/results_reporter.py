# =============== INICIO ARCHIVO: core/reporting/results_reporter.py (v13 - Reporte Automático) ===============
"""
Módulo para generar un reporte final en formato TXT con la configuración
y los resultados de la ejecución (backtest/live).

v13:
- Adaptado para mostrar información específica del modo automático (parámetros UT Bot)
  cuando corresponda, sin alterar el reporte para otros modos.
- Se basa en un `pm_summary` enriquecido que incluye `initial_total_capital`.
"""
import json
import os
import traceback
...
def generate_backtest_report_from_summary(
    ...
    report_lines.append("=" * 80); report_lines.append(f"Reporte de Ejecución - Bot v13".center(80));
    ...
    if pos_enabled:
        ...
        # --- NUEVO: Sección específica para Modo Automático ---
        if "automatic" in operation_mode.lower():
            ...
            report_lines.append(f"    Stop Loss Físico:       {getattr(config_module, 'POSITION_PHYSICAL_STOP_LOSS_PCT', 0.0)}%")
    report_lines.append("")
    ...
    if pos_enabled and pm_summary:
        ...
        if open_long_count > 0:
             ...
             for pos in open_long_details:
                 ...
                 report_lines.append(f"      - ID: ...{str(pos.get('id','N/A'))[-6:]}, Entry: {entry_price_val:.{price_prec_report}f}, Size: {size_contracts_val:.{qty_prec_report}f}, TP: {tp_formatted}")
...
# La función `generate_report` ahora es un wrapper más simple que llama a la función principal
...
# =============== FIN ARCHIVO: core/reporting/results_reporter.py (v13 - Reporte Automático) ===============